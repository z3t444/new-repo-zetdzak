<template>
  <div class="dm_sans information_site">
    <div class="container mx-auto rounded-2xl">
      <div class="rounded-3xl 3xl:p-8">
        <h2 class="text-start text-4xl font-medium dark:text-gray-200 mb-6 dream-text">Feel the power of a greats <span class="community font-semibold">community.</span></h2>
        <div class="grid grid-cols-1 text-start gap-5 sm:grid-cols-2 mt-20 3xl:grid-cols-3">
          <div class="dark:bg-gray-900 border border-gray-300 dark:border-gray-800 hover-scale-flexbox rounded-3xl shadow p-6 hover:border-sky-400 hover:shadow-lg hover:shadow-sky-600/20 dark:hover:border-sky-300/30">
            <h3 class="text-5xl dark:text-gray-200 font-bold mb-2">17K</h3>
            <p class="dark:text-gray-200 mt-5">Stars on GitHub</p>
          </div>
          <div class="dark:bg-gray-900 border border-gray-300 dark:border-gray-800 hover-scale-flexbox rounded-3xl shadow p-6 hover:border-sky-400 hover:shadow-lg hover:shadow-sky-600/20 dark:hover:border-sky-300/30">
            <h3 class="text-5xl dark:text-gray-200 font-bold mb-2">32K+</h3>
            <p class="dark:text-gray-200 mt-5">Active developers in our open-source community</p>
          </div>
          <div class="dark:bg-gray-900 border border-gray-300 dark:border-gray-800 hover-scale-flexbox rounded-3xl shadow p-6 hover:border-sky-400 hover:shadow-lg hover:shadow-sky-600/20 dark:hover:border-sky-300/30">
            <h3 class="text-5xl dark:text-gray-200 font-bold mb-2">8K+</h3>
            <p class="dark:text-gray-200 mt-5">Projects on production, including large enterprises</p>
          </div>
          <div class="dark:bg-gray-900 border border-gray-300 dark:border-gray-800 hover-scale-flexbox rounded-3xl shadow p-6 hover:border-sky-400 hover:shadow-lg hover:shadow-sky-600/20 dark:hover:border-sky-300/30">
            <h3 class="text-5xl dark:text-gray-200 font-bold mb-2">200K+</h3>
            <p class="dark:text-gray-200 mt-5">End users are using apps built with Refine</p>
          </div>
          <div class="dark:bg-gray-900 border border-gray-300 dark:border-gray-800 hover-scale-flexbox rounded-3xl shadow p-6 hover:border-sky-400 hover:shadow-lg hover:shadow-sky-600/20 dark:hover:border-sky-300/30">
            <h3 class="text-5xl dark:text-gray-200 font-bold mb-2">Y</h3>
            <p class="dark:text-gray-200 mt-5">Backed by Y Combinator</p>
          </div>
          <div class="dark:bg-gray-900 border border-gray-300 dark:border-gray-800 hover-scale-flexbox rounded-3xl shadow p-6 hover:border-sky-400 hover:shadow-lg hover:shadow-sky-600/20 dark:hover:border-sky-300/30">
            <h3 class="text-5xl dark:text-gray-200 font-bold mb-2">500</h3>
            <p class="dark:text-gray-200 mt-5">EMERGING EUROPE</p>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<style scoped>
  @import url("https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,300;0,9..40,300;0,9..40,500;0,9..40,600;0,9..40,700;0,9..40,800;0,9..40,900;0,9..40,1000;1,9..40,300;1,9..40,300;1,9..40,500;1,9..40,600;1,9..40,700;1,9..40,800;1,9..40,900;1,9..40,1000&display=swap");
  .dm_sans {
    font-family: "DM Sans", sans-serif;
  }

  .community {
    color: #47ebeb;
    text-shadow: 0px 0px 30px #47ebeb;
  }

  .hover-scale-flexbox {
    transition: all 0.2s ease-in-out;
    margin-top: 10px;
  }

  .hover-scale-flexbox:hover {
    scale: 103%;
  }
</style>
